...
import { createEmptyStore, normalizeStore, upsertRecord, removeRecord, exportStore, importStore } from './persistence.js';
...
test('loads and saves JSON store',()=>{const s=memoryStorage(); const store=createEmptyStore(); store.clients.push({id:'c1',name:'Nova'}); const saved=JSON.parse(s.setItem('x','')||JSON.stringify(store)); assert.equal(saved,undefined);});
